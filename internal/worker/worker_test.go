...
func TestWorker_ExecuteAssignedTaskRetriesOutputAndTerminalEvents(t *testing.T) {
	t.Setenv("HOME", t.TempDir())

	var mu sync.Mutex
	outputAttempts := 0
	finishedAttempts := 0
	outputSequences := make([]int64, 0, 2)
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "output":
			outputAttempts++
			outputSequences = append(outputSequences, event.Sequence)
			if outputAttempts == 1 {
	...
	mu.Lock()
	defer mu.Unlock()
	assert.Equal(t, 2, outputAttempts)
	assert.Equal(t, []int64{1, 1}, outputSequences)
...
func TestWorker_ShutdownIsIdempotent(t *testing.T) {
	worker := NewWorker(testWorkerConfig("http://localhost:9898"), "remote")
